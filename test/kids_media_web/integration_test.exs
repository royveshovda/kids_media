defmodule KidsMediaWeb.IntegrationTest do
  ...
  describe "Full user journey" do
    ...
    test "home page is accessible and user-friendly", %{conn: conn} do
      ...
      assert html =~ "button"  # Interactive element
      
      # Check kid-friendly design
      assert html =~ "text-5xl"     # Large text
      assert html =~ "🐆"          # Visual emoji
      assert html =~ "px-20 py-12" # Large touch targets
    ...
    test "subject route is properly configured", %{conn: conn} do
      ...
      assert conn.status in [200, 500]  # 500 expected without API key
    end
  end
  ...
  describe "Performance and UX" do
    test "pages load quickly for good user experience", %{conn: conn} do
      ...
      assert load_time < 1000, "Page took #{load_time}ms to load, which may be too slow for kids"
    end
    ...
    test "layout is optimized for touch interfaces", %{conn: conn} do
      ...
      assert html =~ "py-12"  # Sufficient vertical padding
      assert html =~ "px-20"  # Sufficient horizontal padding
      assert html =~ "text-5xl"  # Large, readable text
    end
  end
end